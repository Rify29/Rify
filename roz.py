...
RIFY = [
            [
                InlineKeyboardRify('الاستخدام', callback_data="HELP_CALLBACK")
            ],
            [
                InlineKeyboardRify('📣 السورس', url='https://t.me/VV449'),
                InlineKeyboardRify('👨‍💻 المطور', url='https://t.me/VV3XE')
            ]
        ]
...
async def start(client, message):
    text = START_TEXT
    reply_markup = InlineKeyboardMarkup(RIFY)
    await message.reply(
        text=text,
...
async def callback_query(client: Client, query: CallbackQuery):
    if query.data=="HELP_CALLBACK":
        TELETIPS_HELP_RIFY = [
            [
                InlineKeyboardRify("رجوع", callback_data="START_CALLBACK")
            ]
            ]
        reply_markup = InlineKeyboardMarkup(TELETIPS_HELP_RIFY)
        try:
        ...
        reply_markup = InlineKeyboardMarkup(TELETIPS_START_RIFY)
        try:
            await query.edit_message_text(
                START_TEXT,
                reply_markup=reply_markup
